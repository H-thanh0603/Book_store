...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (body.action === "reject") {
      await prisma.inventoryAdjustment.update({ where: { id: adj.id }, data: { status: "REJECTED", reviewedBy: auth.userId, reviewedAt: new Date() } });
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      return tx.inventoryAdjustment.update({
        where: { id: adj.id }, data: { status: "APPROVED", reviewedBy: auth.userId, reviewedAt: new Date() },
      });
    });
