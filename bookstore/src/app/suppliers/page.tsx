    ...
    <main className="min-h-screen bg-[#faf7f2] pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  className="bg-[#faf7f2] border border-[#ede5d8] rounded-lg pl-9 pr-3 py-2 text-xs font-medium w-48"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {suppliers.map((s) => (
            <div key={s.id} className="bg-white rounded-2xl border border-[#ede5d8] shadow-xs p-4 space-y-3 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-bold text-[#1c1917] text-sm">{s.name}</h3>
                  <p className="text-[11px] text-slate-500 font-mono">{s.code}</p>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => openEdit(s)} className="p-1.5 rounded-lg bg-[#faf4ea] hover:bg-[#ede5d8] text-[#574431]">
              ...
              <div className="space-y-1.5 text-xs">
                {s.contactName && (
                  <div className="flex items-center gap-2 text-[#574431]">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#faf4ea] text-[#574431]">
              ...
              {s.productPrices.length > 0 && (
                <div className="pt-2 border-t border-[#ede5d8]">
                  <p className="text-[11px] text-slate-500 font-semibold mb-1">Giá nhập gần nhất</p>
                  <div className="space-y-1">
                    {s.productPrices.slice(0, 3).map((p) => (
                      <div key={p.id} className="flex items-center justify-between text-[11px]">
                        <span className="text-[#574431] truncate">{p.variant.product.name}</span>
        ...
        {suppliers.length === 0 && !loading && (
          <div className="bg-white rounded-2xl border border-[#ede5d8] py-12 text-center text-slate-400 text-xs">
          ...
          <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
              <div className="flex items-center justify-between p-4 border-b border-[#ede5d8]">
                <h3 className="font-bold text-[#1c1917]">{editSupplier ? "Sửa nhà cung cấp" : "Thêm nhà cung cấp mới"}</h3>
                <button onClick={() => setShowForm(false)} className="text-slate-400 hover:text-[#574431]"><XCircle className="w-5 h-5" /></button>
              </div>
              <div className="p-4 space-y-3 max-h-[70vh] overflow-y-auto">
                <div>
                  <label className="block text-xs font-semibold text-[#574431] mb-1">Tên nhà cung cấp *</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-[#574431] mb-1">Mã số thuế</label>
                    <input className="w-full bg-[#faf7f2] border border-[#ede5d8] rounded-lg px-3 py-2 text-xs" value={form.taxCode} onChange={(e) => setForm({ ...form, taxCode: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#574431] mb-1">Người liên hệ</label>
                    <input className="w-full bg-[#faf7f2] border border-[#ede5d8] rounded-lg px-3 py-2 text-xs" value={form.contactName} onChange={(e) => setForm({ ...form, contactName: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-[#574431] mb-1">SĐT</label>
                    <input className="w-full bg-[#faf7f2] border border-[#ede5d8] rounded-lg px-3 py-2 text-xs" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#574431] mb-1">Email</label>
                    <input className="w-full bg-[#faf7f2] border border-[#ede5d8] rounded-lg px-3 py-2 text-xs" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#574431] mb-1">Địa chỉ</label>
                  <input className="w-full bg-[#faf7f2] border border-[#ede5d8] rounded-lg px-3 py-2 text-xs" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-[#574431] mb-1">Điều khoản TT</label>
                    <input className="w-full bg-[#faf7f2] border border-[#ede5d8] rounded-lg px-3 py-2 text-xs" placeholder="VD: NET30" value={form.paymentTerms} onChange={(e) => setForm({ ...form, paymentTerms: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#574431] mb-1">Lead time (ngày)</label>
                    <input type="number" min={0} className="w-full bg-[#faf7f2] border border-[#ede5d8] rounded-lg px-3 py-2 text-xs" value={form.leadTimeDays} onChange={(e) => setForm({ ...form, leadTimeDays: Number(e.target.value) || 0 })} />
                  </div>
                </div>
                <label className="flex items-center gap-2 text-xs font-semibold text-[#574431]">
                  <input type="checkbox" checked={form.isConsignment} onChange={(e) => setForm({ ...form, isConsignment: e.target.checked })} className="rounded border-[#e8dac5]" />
                  Ký gửi
                </label>
              </div>
              <div className="flex justify-end gap-2 p-4 border-t border-[#ede5d8]">
                ...
                <button onClick={saveSupplier} disabled={!form.name.trim()} className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:bg-[#ede5d8] disabled:text-white/50 text-white text-xs font-semibold">
