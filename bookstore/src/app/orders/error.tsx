...
export default function Error({
  ...
  useEffect(() => {
    trackError(error, "error", { component: "route", action: "error_boundary_orders" });
  }, [error]);
  return (
    <main className="min-h-screen bg-slate-50/60 pb-16 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-3xl border border-slate-200 p-8 text-center space-y-4 shadow-xs">
        ...
        <div>
          <h1 className="font-bold text-slate-900 text-lg">Đơn hàng gặp sự cố</h1>
          <p className="text-xs text-slate-500 mt-1">
            Trang đơn hàng bị lỗi khi tải. Nhấn thử lại — dữ liệu của bạn không bị ảnh hưởng.
          </p>
        </div>
        <button
          onClick={() => reset()}
